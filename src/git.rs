...
use crate::utils::{Result, CriticalError, UserError};
...
pub fn get_commit_hash(commit: &str) -> Result<String> {
    ...
    if !commit.stdout.is_empty() {
        ...
        Err(CriticalError::Command(parse_cmd_output(&commit.stderr)?.into()))
    }
}
...
# and might take some time to show up.", tag);
    ...
    Ok(output.lines().filter(|line| !line.starts_with("#")).collect())
}
...
pub fn create_git_tag(tag_name: &str, tag_body: &str) -> Result<()> {
    ...
    if !hash.stdout.is_empty() {
        ...
        Err(CriticalError::Command(parse_cmd_output(&hash.stderr)?.into()))
    }
}
